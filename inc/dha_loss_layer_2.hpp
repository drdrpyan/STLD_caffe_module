#ifndef TLR_OSO_LOSS_LAYER_2_HPP_
#define TLR_OSO_LOSS_LAYER_2_HPP_
...
class DHALoss2Layer : public LossLayer<Dtype>
{
 private:
  //struct Matching
  //{
 ...
 public:
  explicit DHALoss2Layer(const LayerParameter& param);
...
// inline functions
template <typename Dtype>
inline DHALoss2Layer<Dtype>::DHALoss2Layer(
    const LayerParameter& param) : LossLayer<Dtype>(param) {

}

template <typename Dtype>
inline const char* DHALoss2Layer<Dtype>::type() const {
  return "DHALoss2";
}

template <typename Dtype>
inline int DHALoss2Layer<Dtype>::ExactNumBottomBlobs() const {
  return 3;
}

template <typename Dtype>
inline int DHALoss2Layer<Dtype>::ExactNumTopBlobs() const {
  return -1;
}

template <typename Dtype>
inline int DHALoss2Layer<Dtype>::MinTopBlobs() const {
  return 1;
}

template <typename Dtype>
inline int DHALoss2Layer<Dtype>::MaxTopBlobs() const {
  return 13;
}

template <typename Dtype>
inline Dtype DHALoss2Layer<Dtype>::Sigmoid(Dtype value) const {
  return 1. / (1. + std::exp(-value));
}

template <typename Dtype>
inline int DHALoss2Layer<Dtype>::GetAnchorScoreChannel(int anchor_id) const {
...
template <typename Dtype>
inline int DHALoss2Layer<Dtype>::GetAnchorElemChannel(
...
template <typename Dtype>
inline int DHALoss2Layer<Dtype>::GetClassScoreChannel(
...
template <typename Dtype>
inline int DHALoss2Layer<Dtype>::GetCellIdx(int cell_r, int cell_c) const {
  CHECK_GE(cell_r, 0);
...
template <typename Dtype>
inline void DHALoss2Layer<Dtype>::GetCellOffset(
