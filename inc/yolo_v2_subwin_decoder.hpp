...
template <typename Dtype>
class YOLOV2SubwinSoftmaxDecoder : public YOLOV2SoftmaxDecoder<Dtype>
{
 public:
  YOLOV2SubwinSoftmaxDecoder(YOLOV2Handler<Dtype>* yolo_v2_handler,
                             DetectionNMS<Dtype>* nms,
                             const std::vector<cv::Point>& subwin_offset,
                             bool global_detection);
  YOLOV2SubwinSoftmaxDecoder(YOLOV2Handler<Dtype>* yolo_v2_handler,
...
// template functions
template <typename Dtype>
inline YOLOV2SubwinSoftmaxDecoder<Dtype>::YOLOV2SubwinSoftmaxDecoder(
    YOLOV2Handler<Dtype>* yolo_v2_handler,
    DetectionNMS<Dtype>* nms,
    const std::vector<cv::Point>& subwin_offset,
    bool global_detection) 
...
inline YOLOV2SubwinSoftmaxDecoder<Dtype>::YOLOV2SubwinSoftmaxDecoder(
    ...
    DetectionNMS<Dtype>* nms,
    const std::vector<cv::Point>& subwin_offset,
    bool global_detection, const std::vector<float>& anchor_weight) 
  : YOLOV2SoftmaxDecoder<Dtype>(yolo_v2_handler, nms, anchor_weight), 
    subwin_offset_(subwin_offset), global_detection_(global_detection) {

}

template <typename Dtype>
void YOLOV2SubwinSoftmaxDecoder<Dtype>::Decode(
    const caffe::Blob<Dtype>& yolo_out,
    Dtype conf_threshold, bool bbox_nms,
    std::vector<std::vector<Detection<Dtype> > >* detection) {
  CHECK_EQ(yolo_out.num(), subwin_offset_.size());
  ...
  std::vector<std::vector<Detection<Dtype> > > temp_detection;
  YOLOV2SoftmaxDecoder<Dtype>::Decode(yolo_out, conf_threshold, false,
                                      &temp_detection);
