#ifndef TLR_YOLO_V2_FOCAL_LOSS_V1_LAYER_HPP_
#define TLR_YOLO_V2_FOCAL_LOSS_V1_LAYER_HPP_
...
class YOLOV2FocalLossV1Layer : public LossLayer<Dtype>
{
  enum {NUM_ANCHOR_ELEM = 5};
  enum AnchorChannel {X = 0, Y, W, H, CONF, CLASS_BEGIN};

 public:
  explicit YOLOV2FocalLossV1Layer(const LayerParameter& param);
 ...
 private:
  ...
  Dtype RegressionSigmoidFocalLoss(Dtype value, Dtype target,
                                   Dtype alpha, Dtype gamma) const;
  Dtype RegressionSigmoidFocalLossDiff(Dtype value, Dtype target,
                                       Dtype alpha, Dtype gamma) const;
...
// inline functions
template <typename Dtype>
inline YOLOV2FocalLossV1Layer<Dtype>::YOLOV2FocalLossV1Layer(
    const LayerParameter& param) : LossLayer<Dtype>(param) {

}

template <typename Dtype>
inline const char* YOLOV2FocalLossV1Layer<Dtype>::type() const {
  return "YOLOV2FocalLossV1";
}

template <typename Dtype>
inline int YOLOV2FocalLossV1Layer<Dtype>::ExactNumBottomBlobs() const {
  return 3;
}

template <typename Dtype>
inline int YOLOV2FocalLossV1Layer<Dtype>::ExactNumTopBlobs() const {
  return -1;
}

template <typename Dtype>
inline int YOLOV2FocalLossV1Layer<Dtype>::MinTopBlobs() const {
  return 1;
}

template <typename Dtype>
inline int YOLOV2FocalLossV1Layer<Dtype>::MaxTopBlobs() const {
  return 13;
}


template <typename Dtype>
inline int YOLOV2FocalLossV1Layer<Dtype>::GetAnchorChannel(
...
template <typename Dtype>
inline int YOLOV2FocalLossV1Layer<Dtype>::GetClassChannel(
...
template <typename Dtype>
inline Dtype YOLOV2FocalLossV1Layer<Dtype>::Sigmoid(Dtype value) const {
  return 1. / (1. + std::exp(-value));
}

} // namespace caffe

#endif // !TLR_YOLO_V2_FOCAL_LOSS_V1_LAYER_HPP_