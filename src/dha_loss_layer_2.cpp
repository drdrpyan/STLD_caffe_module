#include "dha_loss_layer_2.hpp"

namespace caffe
{

template <typename Dtype>
void DHALoss2Layer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                  const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                      const vector<Blob<Dtype>*>& top) {
  Clear();
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::InitAnchorMap() {
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::Clear() {
  noobj_loss_ = 0;
  obj_loss_ = 0;
  cls_loss_ = 0;
  coord_loss_ = 0;
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::FindBestMatch(
    const std::vector<cv::Rect_<Dtype> >& true_gt_box,
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ArgSortByCellIdx(
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardPositive(const Blob<Dtype>& input,
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardNegative(const Blob<Dtype>& input,
                                          int n, int cell_r, int cell_c) {
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardNegAnchorScore(
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardPosAnchorScore(
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardAnchorScore(
  ...
  for (int i = 0; i < anchor_score_offset.size(); ++i)
    if(true_anchor == neg_anchor_id_)
      diff_data[anchor_score_offset[i]] = noobj_scale_ * softmax_diff[i];
    else
      diff_data[anchor_score_offset[i]] = obj_scale_ * softmax_diff[i];
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardBBox(
...
template <typename Dtype>
void DHALoss2Layer<Dtype>::ForwardClassScore(const Blob<Dtype>& input,
                                            int n, int cell_r, int cell_c,
...
template <typename Dtype>
Dtype DHALoss2Layer<Dtype>::CalcIoU(
...
template <typename Dtype>
Dtype DHALoss2Layer<Dtype>::CalcOverlap(
...
template <typename Dtype>
cv::Rect_<Dtype> DHALoss2Layer<Dtype>::RawBoxToAnchorRelativeForm(
...
#ifdef CPU_ONLY
STUB_GPU(DHALoss2Layer);
#endif

INSTANTIATE_CLASS(DHALoss2Layer);
REGISTER_LAYER_CLASS(DHALoss2);
} // namespace caffe