#include "yolo_v2_focal_loss_v1_layer.hpp"
...
void YOLOV2FocalLossV1Layer<Dtype>::LayerSetUp(
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::Reshape(
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::Forward_cpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::Clear() {
  noobj_loss_ = 0;
  obj_loss_ = 0;
  cls_loss_ = 0;
  coord_loss_ = 0;
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::ShiftAnchors(
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::FindBestMatching(
...
template <typename Dtype>
cv::Rect_<Dtype> YOLOV2FocalLossV1Layer<Dtype>::RawBBoxToAnchorBBox(
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::ForwardNegative(
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::ForwardPositive(
    const Blob<Dtype>& input, int n, int h, int w, int anchor,
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::ForwardConf(
  ...
  //Dtype sub = sig_conf - iou;
  //Dtype current_loss = scale * focal_loss_alpha_ * std::pow(1 - sub, focal_loss_gamma_) * std::log(std::abs(sub));
  //loss += std::abs(current_loss);

  //Dtype entrophy_diff = focal_loss_gamma_ * std::pow(1 - sub, focal_loss_gamma_ - 1) * std::log(std::abs(sub)) + std::pow(1 - sub, focal_loss_gamma_) * (1. / sub);
  //*(diff_data + conf_offset) = scale * entrophy_diff * sig_conf * (1 - sig_conf);
  ////*(diff_data + conf_offset) = scale * (sig_conf - iou) * sig_conf * (1 - sig_conf);
  Dtype current_loss = RegressionSigmoidFocalLoss(conf, iou, 
                                                  focal_loss_alpha_, 
                                                  focal_loss_gamma_);
  loss += scale * current_loss;
  Dtype current_diff = RegressionSigmoidFocalLossDiff(conf, iou,
                                                      focal_loss_alpha_,
                                                      focal_loss_gamma_);
  *(diff_data + conf_offset) = scale * current_diff;
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::ForwardBBox(
    const Blob<Dtype>& input, int n, int h, int w, int anchor,
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::ForwardClass(
...
template <typename Dtype>
void YOLOV2FocalLossV1Layer<Dtype>::Backward_cpu(
    const vector<Blob<Dtype>*>& top, 
...
template <typename Dtype>
cv::Point YOLOV2FocalLossV1Layer<Dtype>::GetCellTopLeft(int row, int col) const {
  CHECK_GE(row, 0);
...
template <typename Dtype>
Dtype YOLOV2FocalLossV1Layer<Dtype>::CalcIoU(
...
template <typename Dtype>
Dtype YOLOV2FocalLossV1Layer<Dtype>::CalcOverlap(
...
template <typename Dtype>
cv::Rect_<Dtype> YOLOV2FocalLossV1Layer<Dtype>::GetPredBBox(
...
template <typename Dtype>
cv::Rect_<Dtype> YOLOV2FocalLossV1Layer<Dtype>::RawBoxToYOLOBox(
...
template <typename Dtype>
cv::Rect_<Dtype> YOLOV2FocalLossV1Layer<Dtype>::YOLOBoxToRawBox(
...
template <typename Dtype>
Dtype YOLOV2FocalLossV1Layer<Dtype>::RegressionSigmoidFocalLoss(
    Dtype value, Dtype target, Dtype alpha, Dtype gamma) const {
  CHECK_GE(target, 0);
  CHECK_LE(target, 1);
  CHECK_GT(alpha, 0);
  CHECK_GE(gamma, 0);

  Dtype abs_sub = std::max(std::abs(Sigmoid(value) - target),
                           std::numeric_limits<Dtype>::min());
  return -alpha * std::pow(abs_sub, gamma) * std::log(1 - abs_sub);
}

template <typename Dtype>
Dtype YOLOV2FocalLossV1Layer<Dtype>::RegressionSigmoidFocalLossDiff(
    Dtype value, Dtype target, Dtype alpha, Dtype gamma) const {
  //CHECK_GE(target, 0);
  //CHECK_LE(target, 1);
  //CHECK_GT(alpha, 0);
  ...
  //Dtype sig = Sigmoid(value);
  //Dtype abs_sub = std::max(std::abs(sig - target),
  //                         std::numeric_limits<Dtype>::min());
  //Dtype term1 = alpha * gamma * std::pow(1 - abs_sub, gamma - 1) * std::log(abs_sub);
  //Dtype term2 = -alpha * std::pow(1 - abs_sub, gamma) / abs_sub;
  //Dtype diff = (sig >= target) ? term1 + term2 : -(term1 + term2);
  //diff *= sig * (1 - sig);
  //return diff;

  return Sigmoid(value) - target;
}

#ifdef CPU_ONLY
STUB_GPU(YOLOV2FocalLossV1Layer);
#endif

INSTANTIATE_CLASS(YOLOV2FocalLossV1Layer);
REGISTER_LAYER_CLASS(YOLOV2FocalLossV1);
